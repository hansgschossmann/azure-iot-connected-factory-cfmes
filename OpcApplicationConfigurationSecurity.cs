namespace CfMes
{
    ...
    public partial class OpcApplicationConfiguration
    {
        ...
        public async Task InitApplicationSecurityAsync()
        {
            ...
            _logger.Information($"Trusted Issuer store type is: {ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StoreType}");
            _logger.Information($"Trusted Issuer Certificate store path is: {ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StorePath}");
            ...
            _logger.Information($"Trusted Peer Certificate store type is: {ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StoreType}");
            _logger.Information($"Trusted Peer Certificate store path is: {ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath}");
            ...
            _logger.Information($"Rejected certificate store type is: {ApplicationConfiguration.SecurityConfiguration.RejectedCertificateStore.StoreType}");
            _logger.Information($"Rejected Certificate store path is: {ApplicationConfiguration.SecurityConfiguration.RejectedCertificateStore.StorePath}");
            ...
            _logger.Information($"Rejection of SHA1 signed certificates is {(ApplicationConfiguration.SecurityConfiguration.RejectSHA1SignedCertificates ? "enabled" : "disabled")}");
            ...
            _logger.Information($"Minimum certificate key size set to {ApplicationConfiguration.SecurityConfiguration.MinimumCertificateKeySize}");
            ...
            _logger.Information($"Application Certificate store type is: {ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate.StoreType}");
            _logger.Information($"Application Certificate store path is: {ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate.StorePath}");
            _logger.Information($"Application Certificate subject name is: {ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate.SubjectName}");
            ...
            if (certificate == null)
            {
                _logger.Information($"No existing Application certificate found. Create a self-signed Application certificate valid from yesterday for {CertificateFactory.DefaultLifeTime} months,");
                _logger.Information($"with a {CertificateFactory.DefaultKeySize} bit key and {CertificateFactory.DefaultHashSize} bit hash.");
                certificate = CertificateFactory.CreateCertificate(
                    ...
                    _logger.Information($"Application certificate with thumbprint '{certificate.Thumbprint}' created.");
            ...
            {
                _logger.Information($"Application certificate with thumbprint '{certificate.Thumbprint}' found in the application certificate store.");
            }
            ApplicationConfiguration.ApplicationUri = Utils.GetApplicationUriFromCertificate(certificate);
            _logger.Information($"Application certificate is for ApplicationUri '{ApplicationConfiguration.ApplicationUri}', ApplicationName '{ApplicationConfiguration.ApplicationName}' and Subject is '{ApplicationConfiguration.ApplicationName}'");
            ...
            if (TrustMyself)
            {
                ...
                try
                {
                    ...
                    _logger.Information($"Adding server certificate to trusted peer store. StorePath={ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath}");
                    await trustedStore.Add(certificate).ConfigureAwait(false);
                }
                catch (Exception e)
                {
                    _logger.Warning(e, "Can not add server certificate to trusted peer store. Maybe it is already there.");
                }
            }
        ...
        public static async Task ShowCreateSigningRequestInformationAsync(X509Certificate2 certificate)
        {
            try
            {
                ...
                if (!certificate.HasPrivateKey)
                {
                    ...
                    {
                        _logger.Error(e, "Error while loading private key.");
                        return;
                    }
                }
                ...
                {
                    _logger.Error(e, "Error while creating signing request.");
                    return;
                }
                _logger.Information("----------------------- CreateSigningRequest information ------------------");
                _logger.Information($"ApplicationUri: {ApplicationConfiguration.ApplicationUri}");
                _logger.Information($"ApplicationName: {ApplicationConfiguration.ApplicationName}");
                _logger.Information($"ApplicationType: {ApplicationConfiguration.ApplicationType}");
                _logger.Information($"ProductUri: {ApplicationConfiguration.ProductUri}");
                if (ApplicationConfiguration.ApplicationType != ApplicationType.Client)
                {
                    ...
                    {
                        _logger.Information($"DiscoveryUrl[{serverNum++}]: {endpoint}");
                    }
                    ...
                    _logger.Information($"ServerCapabilities: {string.Join(", ", serverCapabilities)}");
                }
                _logger.Information("CSR (base64 encoded):");
                ...
                try
                {
                    ...
                    _logger.Information($"Binary CSR written to '{ApplicationConfiguration.ApplicationName}.csr'");
                }
                catch (Exception e)
                {
                    _logger.Error(e, "Error while writing .csr file.");
                }
            }
            catch (Exception e)
            {
                _logger.Error(e, "Error in CSR creation");
            }
        }
        ...
        public static async Task ShowCertificateStoreInformationAsync()
        {
            ...
            try
            {
                ...
                int certNum = 1;
                _logger.Information($"Trusted issuer store contains {certs.Count} certs");
                foreach (var cert in certs)
                {
                    _logger.Information($"{certNum++:D2}: Subject '{cert.Subject}' (thumbprint: {cert.GetCertHashString()})");
                }
                if (certStore.SupportsCRLs)
                {
                    var crls = certStore.EnumerateCRLs();
                    int crlNum = 1;
                    _logger.Information($"Trusted issuer store has {crls.Count} CRLs.");
                    foreach (var crl in certStore.EnumerateCRLs())
                    {
                        _logger.Information($"{crlNum++:D2}: Issuer '{crl.Issuer}', Next update time '{crl.NextUpdateTime}'");
                    }
                }
            }
            catch (Exception e)
            {
                _logger.Error(e, "Error while trying to read information from trusted issuer store.");
            }
            ...
            try
            {
                ...
                var certs = await certStore.Enumerate().ConfigureAwait(false);
                int certNum = 1;
                _logger.Information($"Trusted peer store contains {certs.Count} certs");
                foreach (var cert in certs)
                {
                    _logger.Information($"{certNum++:D2}: Subject '{cert.Subject}' (thumbprint: {cert.GetCertHashString()})");
                }
                if (certStore.SupportsCRLs)
                {
                    var crls = certStore.EnumerateCRLs();
                    int crlNum = 1;
                    _logger.Information($"Trusted peer store has {crls.Count} CRLs.");
                    foreach (var crl in certStore.EnumerateCRLs())
                    {
                        _logger.Information($"{crlNum++:D2}: Issuer '{crl.Issuer}', Next update time '{crl.NextUpdateTime}'");
                    }
            ...
            {
                _logger.Error(e, "Error while trying to read information from trusted peer store.");
            }
            ...
            try
            {
                ...
                var certs = await certStore.Enumerate().ConfigureAwait(false);
                int certNum = 1;
                _logger.Information($"Rejected certificate store contains {certs.Count} certs");
                foreach (var cert in certs)
                {
                    _logger.Information($"{certNum++:D2}: Subject '{cert.Subject}' (thumbprint: {cert.GetCertHashString()})");
                }
            ...
            {
                _logger.Error(e, "Error while trying to read information from rejected certificate store.");
            }
        }
        ...
        private static void CertificateValidator_CertificateValidation(CertificateValidator validator, CertificateValidationEventArgs e)
       {
            if (e.Error.StatusCode == Opc.Ua.StatusCodes.BadCertificateUntrusted)
            {
                ...
                if (AutoAcceptCerts)
                {
                    _logger.Information($"Certificate '{e.Certificate.Subject}' will be trusted, because of corresponding command line option.");
                }
                else
                {
                    ...
                    _logger.Information($"{OpcApplicationConfiguration.ApplicationConfiguration.SecurityConfiguration.RejectedCertificateStore.StorePath}/certs");
                    _logger.Information("to the directory:");
                    _logger.Information($"{OpcApplicationConfiguration.ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath}/certs");
        ...
        private async Task<bool> RemoveCertificatesAsync(List<string> thumbprintsToRemove)
        {
            ...
            if (thumbprintsToRemove.Count == 0)
            {
                _logger.Error("There is no thumbprint specified for certificates to remove. Please check your command line options.");
                return false;
            }
            ...
            try
            {
                _logger.Information("Starting to remove certificate(s) from trusted peer and trusted issuer store.");
                ...
                foreach (var thumbprint in thumbprintsToRemove)
                {
                    ...
                    if (certToRemove?.Count > 0)
                    {
                        ...
                        {
                            _logger.Warning($"Failed to remove certificate with thumbprint '{thumbprint}' from the trusted peer store.");
                        }
                        else
                        {
                            _logger.Information($"Removed certificate with thumbprint '{thumbprint}' from the trusted peer store.");
                        }
                    }
                }
            }
            ...
            {
                _logger.Error(e, "Error while trying to remove certificate(s) from the trusted peer store.");
                result = false;
            }
            ...
            try
            {
                ...
                foreach (var thumbprint in thumbprintsToRemove)
                {
                    ...
                    if (certToRemove?.Count > 0)
                    {
                        ...
                        {
                            _logger.Warning($"Failed to delete certificate with thumbprint '{thumbprint}' from the trusted issuer store.");
                        }
                        else
                        {
                            _logger.Information($"Removed certificate with thumbprint '{thumbprint}' from the trusted issuer store.");
                        }
                    }
                }
            }
            ...
            {
                _logger.Error(e, "Error while trying to remove certificate(s) from the trusted issuer store.");
                result = false;
            }
            return result;
        }
            ...
            bool issuerCertificate = true)
        {
            ...
            {
                _logger.Error("There is no certificate provided. Please check your command line options.");
                return false;
            }

            _logger.Information($"Starting to add certificate(s) to the {(issuerCertificate ? "trusted issuer" : "trusted peer")} store.");
            ...
            try
            {
                ...
                if (certificateBase64Strings?.Count > 0)
                {
                    foreach (var certificateBase64String in certificateBase64Strings)
                    {
                        ...
                        {
                            _logger.Error($"The provided string '{certificateBase64String.Substring(0, 10)}...' is not a valid base64 string.");
                            return false;
                        }
                    }
                }
            ...
            {
                _logger.Error(e, "The issuer certificate data is invalid. Please check your command line options.");
                return false;
            }
                ...
                try
                {
                    ...
                    foreach (var certificateToAdd in certificatesToAdd)
                    {
                        try
                        {
                            ...
                            _logger.Information($"Certificate '{certificateToAdd.SubjectName.Name}' and thumbprint '{certificateToAdd.Thumbprint}' was added to the trusted issuer store.");
                        }
                        catch (ArgumentException)
                        {
                            // ignore error if cert already exists in store
                            _logger.Information($"Certificate '{certificateToAdd.SubjectName.Name}' already exists in trusted issuer store.");
                        }
                    }
                }
                catch (Exception e)
                {
                    _logger.Error(e, "Error while adding a certificate to the trusted issuer store.");
                    result = false;
                }
            }
            else
                ...
                try
                {
                    ...
                    foreach (var certificateToAdd in certificatesToAdd)
                    {
                        try
                        {
                            ...
                            _logger.Information($"Certificate '{certificateToAdd.SubjectName.Name}' and thumbprint '{certificateToAdd.Thumbprint}' was added to the trusted peer store.");
                        }
                        catch (ArgumentException)
                        {
                            // ignore error if cert already exists in store
                            _logger.Information($"Certificate '{certificateToAdd.SubjectName.Name}' already exists in trusted peer store.");
                        }
                    }
                }
                catch (Exception e)
                {
                    _logger.Error(e, "Error while adding a certificate to the trusted peer store.");
                    result = false;
                }
            }
            return result;
        ...
        private async Task<bool> UpdateCrlAsync(string newCrlBase64String, string newCrlFileName)
        {
            ...
            {
                _logger.Error("There is no CRL specified. Please check your command line options.");
                return false;
            }

            // validate input and create the new CRL
            _logger.Information("Starting to update the current CRL.");
            X509CRL newCrl;
            try
            {
                if (string.IsNullOrEmpty(newCrlFileName))
                {
                    ...
                    {
                        _logger.Error($"The provided string '{newCrlBase64String.Substring(0, 10)}...' is not a valid base64 string.");
                        return false;
                    }
                }
                else
            ...
            {
                _logger.Error(e, "The new CRL data is invalid.");
                return false;
            }
            ...
            using (ICertificateStore trustedStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedPeerCertificates.StorePath))
            {
                ...
                foreach (var trustedCertificate in trustedCertificates)
                {
                    try
                    {
                        if (Utils.CompareDistinguishedName(newCrl.Issuer, trustedCertificate.Subject) && newCrl.VerifySignature(trustedCertificate, false))
                        {
                            ...
                            _logger.Information("Remove the current CRL from the trusted peer store.");
                            ...
                            foreach (var crlToRemove in crlsToRemove)
                            {
                                try
                                {
                                    ...
                                    {
                                        _logger.Warning($"Failed to remove CRL issued by '{crlToRemove.Issuer}' from the trusted peer store.");
                                    }
                                }
                                catch (Exception e)
                                {
                                    _logger.Error(e, $"Error while removing the current CRL issued by '{crlToRemove.Issuer}' from the trusted peer store.");
                                    result = false;
                                }
                            }
                        }
                    ...
                    {
                        _logger.Error(e, "Error while removing the cureent CRL from the trusted peer store.");
                        result = false;
                    }
                }
                ...
                if (trustedCrlIssuer)
                {
                    try
                    {
                        ...
                        _logger.Information($"The new CRL issued by '{newCrl.Issuer}' was added to the trusted peer store.");
                    }
                    catch (Exception e)
                    {
                        _logger.Error(e, "Error while adding the new CRL to the trusted peer store.");
                        result = false;
                    }
                }
            }
            ...
            using (ICertificateStore issuerStore = CertificateStoreIdentifier.OpenStore(ApplicationConfiguration.SecurityConfiguration.TrustedIssuerCertificates.StorePath))
            {
                ...
                foreach (var issuerCertificate in issuerCertificates)
                {
                    try
                    {
                        ...
                        {
                            // the issuer of the new CRL is trusted. delete the crls of the issuer in the trusted store
                            _logger.Information("Remove the current CRL from the trusted issuer store.");
                            trustedCrlIssuer = true;
                            ...
                            foreach (var crlToRemove in crlsToRemove)
                            {
                                try
                                {
                                    ...
                                    {
                                        _logger.Warning($"Failed to remove the current CRL issued by '{crlToRemove.Issuer}' from the trusted issuer store.");
                                    }
                                }
                                catch (Exception e)
                                {
                                    _logger.Error(e, $"Error while removing the current CRL issued by '{crlToRemove.Issuer}' from the trusted issuer store.");
                                    result = false;
                                }
                            }
                        }
                    ...
                    {
                        _logger.Error(e, "Error while removing the current CRL from the trusted issuer store.");
                        result = false;
                    }
                }
                ...
                if (trustedCrlIssuer)
                {
                    try
                    {
                        ...
                        _logger.Information($"The new CRL issued by '{newCrl.Issuer}' was added to the trusted issuer store.");
                    }
                    catch (Exception e)
                    {
                        _logger.Error(e, $"Error while adding the new CRL issued by '{newCrl.Issuer}' to the trusted issuer store.");
                        result = false;
                    }
                }
            }
            ...
            string privateKeyFileName)
        {
            ...
            {
                _logger.Error("There is no new application certificate data provided. Please check your command line options.");
                return false;
            }
            ...
            try
            {
                if (string.IsNullOrEmpty(newCertificateFileName))
                {
                    ...
                    {
                        _logger.Error($"The provided string '{newCertificateBase64String.Substring(0, 10)}...' is not a valid base64 string.");
                        return false;
                    }
                }
                else
            ...
            {
                _logger.Error(e, "The new application certificate data is invalid.");
                return false;
            }

            // validate input and create the private key
            _logger.Information("Start updating the current application certificate.");
            ...
            try
            {
                if (!string.IsNullOrEmpty(privateKeyBase64String))
                {
                    ...
                    {
                        _logger.Error($"The provided string '{privateKeyBase64String.Substring(0, 10)}...' is not a valid base64 string.");
                        return false;
                    }
                }
            ...
            {
                _logger.Error(e, "The private key data is invalid.");
                return false;
            }
            ...
            if (ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate?.Certificate != null)
            {
                ...
                _logger.Information($"The current application certificate has SubjectName '{currentSubjectName}' and thumbprint '{currentApplicationCertificate.Thumbprint}'.");
            }
            else
            {
                _logger.Information("There is no existing application certificate.");
            }
            ...
            {
                _logger.Error($"The SubjectName '{newCertificate.SubjectName.Name}' of the new certificate doesn't match the current certificates SubjectName '{currentSubjectName}'.");
                return false;
            }
            ...
            {
                _logger.Error(e, "Failed to verify integrity of the new certificate and the trusted issuer list.");
                return false;
            }
            ...
            if (string.IsNullOrEmpty(newCertFormat))
            {
                try
                {
                    ...
                    newCertFormat = "PFX";
                    _logger.Information("The private key for the new certificate was passed in using PFX format.");
                }
                catch
                {
                    _logger.Debug("Certificate file is not PFX");
                }
            }
            ...
            if (string.IsNullOrEmpty(newCertFormat))
            {
                try
                {
                    ...
                    newCertFormat = "PEM";
                    _logger.Information("The private key for the new certificate was passed in using PEM format.");
                }
                catch
                {
                    _logger.Debug("Certificate file is not PEM");
                }
            }
            if (string.IsNullOrEmpty(newCertFormat))
            {
                ...
                try
                {
                    ...
                    {
                        _logger.Error("There is no existing application certificate we can use to extract the private key. You need to pass in a private key using PFX or PEM format.");
                    }
                }
                catch
                {
                    _logger.Debug("Application certificate format is not DER");
                }
            }
            ...
            if (hasApplicationCertificate)
            {
                if (string.IsNullOrEmpty(newCertFormat))
                {
                    _logger.Error("The provided format of the private key is not supported (must be PEM or PFX) or the provided cert password is wrong.");
                    return false;
                }
            }
            else
            {
                if (string.IsNullOrEmpty(newCertFormat))
                {
                    _logger.Error("There is no application certificate we can update and for the new application certificate there was not usable private key (must be PEM or PFX format) provided or the provided cert password is wrong.");
                    return false;
                }
            }
            ...
            {
                _logger.Information("Remove the existing application certificate.");
                try
                {
                    if (hasApplicationCertificate && !await appStore.Delete(currentApplicationCertificate.Thumbprint).ConfigureAwait(false))
                    {
                        _logger.Warning($"Removing the existing application certificate with thumbprint '{currentApplicationCertificate.Thumbprint}' failed.");
                    }
                }
                catch
                {
                    _logger.Warning("Failed to remove the existing application certificate from the ApplicationCertificate store.");
                }
                try
                {
                    await appStore.Add(newCertificateWithPrivateKey).ConfigureAwait(false);
                    _logger.Information($"The new application certificate '{newCertificateWithPrivateKey.SubjectName.Name}' and thumbprint '{newCertificateWithPrivateKey.Thumbprint}' was added to the application certificate store.");
                }
                catch (Exception e)
                {
                    _logger.Error(e, "Failed to add the new application certificate to the application certificate store.");
                    return false;
                }
            }
            ...
            try
            {
                _logger.Information($"Activating the new application certificate with thumbprint '{newCertificateWithPrivateKey.Thumbprint}'.");
                ApplicationConfiguration.SecurityConfiguration.ApplicationCertificate.Certificate = newCertificate;
                await ApplicationConfiguration.CertificateValidator.UpdateCertificate(ApplicationConfiguration.SecurityConfiguration).ConfigureAwait(false);
            }
            catch (Exception e)
            {
                _logger.Error(e, "Failed to activate the new application certificate.");
                return false;
            }
